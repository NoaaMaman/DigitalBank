...
contract BankAccount {
    ...
    struct WithdrawRequest {
        address user;
        uint256 amount;
        uint256 approvals;//num_of_approvals
        //Which ownrs approve the withdraw for us
        mapping(address => bool) ownersApproved;
        bool approved;
    }
    //We mapping key-values -> the key is unsigned 256-bit integers 
    //and the values are "withdrawRequest" struct 
    //We are mapping wothdrawl requests with the integer  key being a unique identifiler for each request
    struct Account {
        address[] owners;
        uint256 balance;
        mapping(uint256 => WithdrawRequest) withdrawRequests;
    }
    //connect the accounts to the unique ID
    mapping(uint256 => Account) accounts;
    //Eahch user need to know which account is owner
    mapping(address => uint256[]) userAccounts;

    //Auxilary valiables for creating the ID's for accounts and withdrawls 
    ...
    //The function can be called only from theowner of the account
    //This function alllows an account owner to deposite ether into 
    //their account by calling the following function 
